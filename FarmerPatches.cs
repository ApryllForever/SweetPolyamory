    ...
    public static class FarmerPatches
    {
        ...
        public static bool Farmer_doDivorce_Prefix(ref Farmer __instance)
        {
            try
            {
                ...
                if (!__instance.isMarriedOrRoommates() || Mod.spouseToDivorce == null)
                {
                    Monitor.Log("Tried to divorce but no spouse to divorce!");
                    return false;
                }

                string key = Mod.spouseToDivorce;

                int points = 2000;
                if (Mod.divorceHeartsLost <= 0)
                {
                    points = 0;
                }
                else
                {
                    points -= Mod.divorceHeartsLost * 250;
                }
                ...
                if (__instance.friendshipData.ContainsKey(key))
                {
                    ...
                    Mod.currentSpouses.Remove(__instance.UniqueMultiplayerID);
                    Mod.currentUnofficialSpouses.Remove(__instance.UniqueMultiplayerID);
                    Mod.ResetSpouses(__instance);
        ...
        public static bool Farmer_isMarried_Prefix(Farmer __instance, ref bool __result)
        {
            try
            {
                __result = __instance.team.IsMarried(__instance.UniqueMultiplayerID) || Mod.GetSpouses(__instance, true).Count > 0;
                return false;
            }
            catch (Exception ex)
            {
        ...
        public static void Farmer_spouse_Postfix(Farmer __instance, ref string __result)
        {
            ...
            {
                skipSpouse = true;
                if (Mod.tempOfficialSpouse != null && __instance.friendshipData.ContainsKey(Mod.tempOfficialSpouse.Name) && __instance.friendshipData[Mod.tempOfficialSpouse.Name].IsMarried())
                {
                    __result = Mod.tempOfficialSpouse.Name;
                }
                else
                {
                    var spouses = Mod.GetSpouses(__instance, true);
                    string aspouse = null;
        ...
        public static void Farmer_getSpouse_Postfix(Farmer __instance, ref NPC __result)
        {
            try
            {
                ...
                if (Mod.tempOfficialSpouse != null && __instance.friendshipData.ContainsKey(Mod.tempOfficialSpouse.Name) && __instance.friendshipData[Mod.tempOfficialSpouse.Name].IsMarried())
                {
                    __result = Mod.tempOfficialSpouse;
                }
                else
                {
                    var spouses = Mod.GetSpouses(__instance, true);
        ...
        public static bool Farmer_GetSpouseFriendship_Prefix(Farmer __instance, ref Friendship __result)
        {
            try
            {
                ...
                if (Mod.tempOfficialSpouse != null && __instance.friendshipData.ContainsKey(Mod.tempOfficialSpouse.Name) && __instance.friendshipData[Mod.tempOfficialSpouse.Name].IsMarried())
                {
                    __result = __instance.friendshipData[Mod.tempOfficialSpouse.Name];
                    return false;
                }
            }
            catch (Exception ex)
