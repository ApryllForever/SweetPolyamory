...
namespace SweetPolyamory
{
    public class PathFindControllerPatches
    {
        ...
        public static void PathFindController_Prefix(Character c, GameLocation location, ref Point endPoint)
        {
            try
            {
                if ( !(c is NPC) || !(c as NPC).isVillager() || !(c as NPC).isMarried() || !(location is FarmHouse) || endPoint == (location as FarmHouse).getEntryLocation())
                    return;

                if (Mod.IsInBed(location as FarmHouse, new Rectangle(endPoint.X * 64, endPoint.Y * 64, 64, 64)))
                {
                    Point point = Mod.GetSpouseBedEndPoint(location as FarmHouse, c.Name);
