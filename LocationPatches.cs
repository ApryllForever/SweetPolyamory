    ...
    public static class LocationPatches
    {
        ...
        public static bool FarmHouse_getSpouseBedSpot_Prefix(FarmHouse __instance, string spouseName, ref Point __result)
        {
            try
            {
                if (spouseName == null || !Config.EnableMod)
                    return true;
                var spouses = Mod.GetSpouses(__instance.owner, true);
                
                if (!spouses.TryGetValue(spouseName, out NPC spouse) || spouse is null || spouse.isMoving() || !Mod.IsInBed(__instance, spouse.GetBoundingBox()))
                    return true;
        ...
        public static void Beach_resetLocalState_Postfix(Beach __instance)
        {
            try
            {
                ...
                if (Mod.Config.BuyPendantsAnytime)
                {
                    Mod.SHelper.Reflection.GetField<NPC>(__instance, "oldMariner").SetValue(new NPC(new AnimatedSprite("Characters\\Mariner", 0, 16, 32), new Vector2(80f, 5f) * 64f, 2, "Old Mariner", null));
                }
            }
            catch (Exception ex)
            {
        ...
        public static void GameLocation_checkEventPrecondition_Prefix(ref string precondition)
        {
            try
            {
                ...
                Dictionary<string, NPC> spouses = Mod.GetSpouses(Game1.player, true);
        ...
        public static bool ManorHouse_performAction_Prefix(ManorHouse __instance, string action, Farmer who, ref bool __result)
        {
            try
            {
                ...
                Dictionary<string, NPC> spouses = Mod.GetSpouses(who, true);
