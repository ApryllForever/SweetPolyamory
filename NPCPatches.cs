    ...
    public static class NPCPatches
    {
        ...
        internal static void NPC_GetDispositionModifiedString_Prefix(NPC __instance, ref bool __state)
        {
            try
            {
                ...
                {
                    Mod.tempOfficialSpouse = __instance;
                    __state = true;
                }
            }
            catch (Exception ex)
        ...
        public static void NPC_marriageDuties_Prefix(NPC __instance)
        {
            try
            {
                if (Mod.GetSpouses(Game1.player, false).ContainsKey(__instance.Name))
                {
                    Mod.tempOfficialSpouse = __instance;
                }
            }
        ...
        public static void NPC_marriageDuties_Postfix(NPC __instance)
        {
            try
            {
                if (Mod.tempOfficialSpouse == __instance)
                {
                    Mod.tempOfficialSpouse = null;
                }
                return;
                ...
                for (int i = 0; i < __instance.currentMarriageDialogue.Count; i++)
                {
                    ...
                    if (mdr.DialogueFile == "Strings\\StringsFromCSFiles")
                    {
                        ...
                        {
                            string key = array[Mod.myRand.Next(0, array.Length)];
                            if (array.Contains(key))
                            {
                                Dictionary<string, string> marriageDialogues = null;
                                try
                                {
                                    marriageDialogues = Mod.SHelper.GameContent.Load<Dictionary<string, string>>("Characters\\Dialogue\\MarriageDialogue" + __instance.Name);
                                }
                                catch (Exception)
                                {
                                }
...
*/
        ...
        public static bool NPC_checkAction_Prefix(ref NPC __instance, ref Farmer who, GameLocation l, ref bool __result)
        {
            ...
            try
            {
                ...
                if ((__instance.Name.Equals(who.spouse) || Mod.GetSpouses(who, true).ContainsKey(__instance.Name)) && __instance.Sprite.CurrentAnimation == null && who.IsLocalPlayer)
                {
                    ...
                    if (__instance.Sprite.CurrentAnimation == null && !__instance.hasTemporaryMessageAvailable() && __instance.currentMarriageDialogue.Count == 0 && __instance.CurrentDialogue.Count == 0 && Game1.timeOfDay < 2200 && !__instance.isMoving() && who.ActiveObject == null )
                    {
                        ...
                        if (__instance.FacingDirection == 3 || __instance.FacingDirection == 1)
                        {
                            ...
                            if (who.getFriendshipHeartLevelForNPC(__instance.Name) >= 9)
                            {
                                ...
                                if (!__instance.hasBeenKissedToday.Value)
                                {
                                    ...
                                    if (who.friendshipData[__instance.Name].RoommateMarriage)
                                    {
                                        ...
                                        Mod.mp.broadcastSprites(who.currentLocation, new TemporaryAnimatedSprite[]
                                        {
                            ...
                            else
                            {
                                ...
                                __instance.faceDirection((Mod.myRand.NextDouble() < 0.5) ? 2 : 0);
        ...
        public static bool NPC_tryToReceiveActiveObject_Prefix(NPC __instance, ref Farmer who, Dictionary<string, string> ___dialogue)
        {
            try
            {
                ...
                else if (who.ActiveObject.ParentSheetIndex == 458)    // Delta HERE Change Fix FUCKITALL        ////////////////
                {
                    Monitor.Log($"Try give bouquet to {__instance.Name}");

                    if (Mod.GetSpouses(who, true).ContainsKey(__instance.Name) && Game1.player.currentLocation.Equals("Farmhouse") )     //   ??? Add if current location = farmhouse?
                    {
                    ...
                    else
                    {
                        ...
                        if (friendship?.IsDating() == false)
                        {
                            ...
                            Multiplayer mp = Mod.SHelper.Reflection.GetField<Multiplayer>(typeof(Game1), "multiplayer").GetValue();
                ...
                else if (who.ActiveObject.ParentSheetIndex == 809 && !who.ActiveObject.bigCraftable.Value)
                {
                    ...
                    if (Mod.GetSpouses(who, true).ContainsKey(__instance.Name) && Utility.doesMasterPlayerHaveMailReceivedButNotMailForTomorrow("ccMovieTheater") && !__instance.Name.Equals("Krobus") && who.lastSeenMovieWeek.Value < Game1.Date.TotalWeeks && !Utility.isFestivalDay(Game1.dayOfMonth, Game1.season) && Game1.timeOfDay <= 2100 && __instance.lastSeenMovieWeek.Value < Game1.Date.TotalWeeks && MovieTheater.GetResponseForMovie(__instance) != "reject")
                    {
                        ...
                        if (who == Game1.player)
                        {
                            Mod.mp.globalChatInfoMessage("MovieInviteAccept", new string[]
                            {
        ...
        public static void Character_displayName_Getter_Postfix(ref Character __instance, ref string __result)
        {
            try
            {
                if (__instance.Name == null || !(__instance is Child) || !Config.ShowParentNames || !__instance.modData.ContainsKey("ApryllForever.SweetPolyamory/OtherParent"))
                    return;
                __result = $"{__result} ({__instance.modData["ApryllForever.SweetPolyamory/OtherParent"]})";
            }
            catch (Exception ex)
            {
